...
class WhatsAppService {
  ...
  // Conexão WhatsApp real com QR Code
  ...
  async disconnectWhatsApp(): Promise<boolean> {
    try {
      ...
      if (this.currentSessionId) {
        qrCodeService.removeSession(this.currentSessionId);
  ...
  // Verificar se o usuário escaneou o QR Code
  private startConnectionCheck(sessionId: string): void {
    this.connectionCheckInterval = setInterval(async () => {
      try {
        // Simular verificação de conexão (em produção, isso seria uma verificação real)
        const sessionInfo = qrCodeService.getSessionInfo(sessionId);
        
        if (sessionInfo) {
          // Simular que o usuário escaneou o QR Code após 10 segundos
          const timeSinceCreation = Date.now() - sessionInfo.timestamp;
  ...
  private async handleSuccessfulConnection(sessionId: string): Promise<void> {
    try {
      ...
      // Limpar sessão do QR Code service
      qrCodeService.removeSession(sessionId);
